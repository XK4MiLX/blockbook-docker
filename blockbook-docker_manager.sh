...
if [[ "$1" == "create" ]]; then
  ...
  CMD=$(echo "docker run -d --name fluxosblockbook-${2} -e COIN=${2} $BINARY_NAME -e BLOCKBOOK_PORT=${BLOCKBOOKPORT} $flage $CONFIG $CLIFLAGS $RPC_URL_PROTOCOL -e RPC_PORT=${RPC_PORT} $POSTINST $EXTRAFLAGS -p ${OUT_PORT}:${BLOCKBOOKPORT} -v /home/$USER/fluxosblockbook_${2}:/root runonflux/blockbook-docker" | awk '$1=$1')
  echo -e "| $CMD"
  bash -c "$CMD"
  echo -e ""
else
  ...
  echo -e "| docker run -d --name fluxosblockbook-${1} -e COIN=${1} $BINARY_NAME -e BLOCKBOOK_PORT=${BLOCKBOOKPORT} $flage $CONFIG $CLIFLAGS $RPC_URL_PROTOCOL -e RPC_PORT=${RPC_PORT} $POSTINST $EXTRAFLAGS -p ${OUT_PORT}:${BLOCKBOOKPORT} -v /home/$USER/fluxosblockbook_${1}:/root runonflux/blockbook-docker" | awk '$1=$1'
  echo -e "----------------------------------------------------------------------------------------"
fi
